    ...
    public void onDragStart(View v, DragSource source, Object info, int dragAction) {
        ...
        if (item != null) {
            ...
            if(mLauncher.isDockBarOpen()){
                ...
                if(!mTrickyLocation){
                    if(mOrientation==ORIENTATION_HORIZONTAL){
                    	mRegion.set(location[0], location[1]-mCustomPadding, location[0] + getRight() - getLeft(),
                            location[1] + getBottom() - getTop()-mCustomPadding);
                    }else{
                    	mRegion.set(location[0]-mCustomPadding, location[1], location[0] + getRight() - getLeft() -mCustomPadding,
